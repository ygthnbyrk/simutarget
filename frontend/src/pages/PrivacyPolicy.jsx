...
function PrivacyPolicy() {
  ...
  const sections = [
    'intro', 'collection', 'usage', 'cookies',
    'thirdParty', 'security', 'retention',
    'rights', 'children', 'changes', 'contact'
  ]
    ...
    <LegalPageLayout>
      <div style={{ textAlign: 'center', marginBottom: '48px' }}>
        ...
        <p style={{ fontSize: '15px', color: 'var(--color-text-muted)' }}>
          {t('privacy.lastUpdated')}: February 2025
        </p>
      </div>
      ...
      <div style={{
        ...
        {sections.map((sec, i) => (
          <div key={sec} style={sectionStyle}>
            <h2 style={headingStyle}>{i + 1}. {t(`privacy.${sec}_title`)}</h2>
            <p style={textStyle}>{t(`privacy.${sec}_text`)}</p>
            {t(`privacy.${sec}_text2`, { defaultValue: '' }) && (
              <p style={textStyle}>{t(`privacy.${sec}_text2`)}</p>
            )}
          </div>
        ))}
      </div>
