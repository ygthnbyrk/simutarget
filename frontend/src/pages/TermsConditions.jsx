...
function TermsConditions() {
  ...
  const sections = [
    'acceptance', 'description', 'accounts', 'subscription',
    'usage', 'intellectual', 'limitation', 'termination',
    'modifications', 'governing', 'contact'
  ]
    ...
    <LegalPageLayout>
      <div style={{ textAlign: 'center', marginBottom: '48px' }}>
        ...
        <p style={{ fontSize: '15px', color: 'var(--color-text-muted)' }}>
          {t('terms.lastUpdated')}: February 2025
        </p>
      </div>
      ...
      <div style={{
        ...
        {sections.map((sec, i) => (
          <div key={sec} style={sectionStyle}>
            <h2 style={headingStyle}>{i + 1}. {t(`terms.${sec}_title`)}</h2>
            <p style={textStyle}>{t(`terms.${sec}_text`)}</p>
            {t(`terms.${sec}_text2`, { defaultValue: '' }) && (
              <p style={textStyle}>{t(`terms.${sec}_text2`)}</p>
            )}
          </div>
        ))}
      </div>
